...
class boolean_function {
 ...
 public:
  ...
  static boolean_function zero(size_t dimension) {
    boolean_function zero;
    dimension = 1 << dimension;
    zero.data.resize(dimension);
    for (size_t i = 0; i < dimension; i++) {
      zero.data[i] = false;
    }
    return zero;
  ...
  static boolean_function one(size_t dimension) {
    boolean_function one;
    dimension = 1 << dimension;
    one.data.resize(dimension);
    for (size_t i = 0; i < dimension; i++) {
      one.data[i] = true;
    }
    return one;
  ...
  explicit boolean_function(const std::initializer_list<bool> vars) {
    size_t n = vars.size();
    size_t t = 0;
    data.resize(n);
    for (auto i : vars) {
  ...
  boolean_function& operator+=(const boolean_function& rhs) {
    if (data.size() != rhs.data.size()) {
      throw std::invalid_argument("No eligible argument provided");
    }
  ...
  bool operator>=(const boolean_function& rhs) const {
    ...
    if (weight() > rhs.weight()) {
      for (size_t i = 0; i < size(); i++) {
        if (data[i] == 0 && rhs.data[i] == 1) {
          throw std::logic_error("Shit");
        }
      }
    } else {
      for (size_t i = 0; i < size(); i++) {
        if (data[i] == 1 && rhs.data[i] == 0) {
          throw std::logic_error("Shit");
        }
      }
    }
  ...
  boolean_function operator()(
    ...
    boolean_function b(a.dimension());
    std::vector<bool> k;
    for (size_t i = 0; i < a.size(); ++i) {
      k.clear();
      for (auto j : vars) {
    ...
    for (size_t i = 0; i < a.size(); i++) {
      for (size_t j = i + 1; j < a.size(); j++) {
        if (a[i] > a[j]) {
          int t = 0;
          int k = 0;
          boolean_function z(i, dimension());
          boolean_function y(j, dimension());
